...
class AIService {
  ...
  async queryOnline(
    ...
    let systemPrompt =
      'You are a Sri Lankan tea agronomy assistant. Provide answers using the knowledge base provided. Always respond in plain text only. Do not use ** for bold, markdown, or any other special text formatting. Avoid bullet points and numbered lists.';
    if (hasMatches) {
      systemPrompt +=
    ...
    const userPrompt = `User Question:\n${query}\n\nProvide a farmer-friendly answer in ${langName}. Do not exaggerate or provide false information. Keep the tone clear and direct. Do not use bold, italics, markdown, or any decorative text styles—just plain text sentences.`;
